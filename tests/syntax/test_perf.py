...
@pytest.mark.perf
def test_resolve_10mb_uniform_under_250ms():
    big = "$var{a|b|c}" * (10_000_000 // 11)
    ctx = _PerfCtx(_vars={"var": "x"})
    elapsed = _measure(lambda: resolve_text(big, ctx))
    assert elapsed < 0.25, f"resolve_text took {elapsed:.3f}s for 10MB"


@pytest.mark.perf
def test_resolve_complex_realistic_under_500ms():
    ...
    elapsed = _measure(lambda: resolve_text(big, ctx))
    assert elapsed < 0.5, f"complex resolve took {elapsed:.3f}s"
