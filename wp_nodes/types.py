...
_CROSS_NODE_INTERNAL_KEYS = (
    ...
    # SP3 reach-selector hit counter ALSO has to propagate — without it,
    # Context B's first/next counters reset (a `first` constraint would
    # re-fire on B's first target instance even though A already spent
    # it), and B re-runs the never_applied end-of-run check against
    # constraints A already applied, re-emitting the same warnings.
...
def build_payload(
    ...
    # the whole chain) or under-reaches a `next N` / `pick` selector,
    # every node along the way emits the same row and the user sees N
    # copies. Keying on (type, module_id) here collapses them to one.
    _DEDUP_CONSTRAINT_WARNINGS = frozenset({
        "constraint_never_applied",
    ...
    for w in combined_warnings:
        if not isinstance(w, dict):
            ...
            continue
        wtype = w.get("type")
        if wtype in _DEDUP_CONSTRAINT_WARNINGS:
            mid = w.get("module_id") or ""
            key = (str(wtype), str(mid))
            if key in seen_keys:
                continue
            seen_keys.add(key)
        deduped.append(w)
