...
/**
 * Constraint IdentitySection — display name only. Mirrors derivation's
 * IdentitySection 1:1 because constraint also produces no single
 * binding (returns empty dict from resolve), so there's no
 * variable-binding row to override. Library-defining edits (matrix,
 * source/target wildcards, exceptions list shape) live in SPA.
 */
